...
版式以 `references/diagrams.md` 为准，本节省略重复的颜色/间距。

- 路径：`docs/agents/diagrams/flows/{domain}-{slug}.svg`
- **主路径单列**；最多 1 条右侧旁路。不要收成「入口→编排→写库」三框
- 节点上限 **16**。仍超：合并纯转发，**判断和写库不要合并**
- V 很宽：画「选择键」菱形 + 主路径一条 + 对照分支一条 + 「其它 N → 见细节页」
- chain/pipeline：按顺序画 4～6 个算子，其余「+N」
- 节点正文：`短类名::方法`、判断短句、或「写 {tableName}」。每节点 ≤ 2 行；字段、`$var`、catch 正文只在 md
- 边上标 是/否、通过/Abort、选择键取值（≤ 6 字）
- 禁止 mermaid 替代 SVG
- 自检：图上每个判断/写库能在细节页表里对上；并通过 `diagrams.md` 落盘清单
